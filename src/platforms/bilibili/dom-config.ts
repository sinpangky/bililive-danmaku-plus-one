...
export const BILIBILI_PREFIX_TEXT_SELECTOR = '.bili-danmaku-x-prefixImage-text-container'
...
export const BILIBILI_CHAT_STRONG_ACTION_TEXT =
  /(?:举报|禁言|关注|取关|拉黑|屏蔽|用户资料|个人主页)/i
...
export function isBilibiliAdvertisementMarker(value: unknown): boolean {
  return /(?:^|[\s_-])(?:ad|ads|advert|advertise|advertisement|commercial|marketing|promo|promote|promotion)(?:$|[\s_-])/i.test(
    String(value || ''),
  )
}

export function isBilibiliAdvertisementLabel(value: unknown): boolean {
  return /^(?:广告|推广|推广内容|赞助|赞助内容|商业推广|活动推荐|推荐活动|去看看|立即查看|了解详情)$/i.test(
    String(value || '')
      .replace(/\s+/g, '')
      .trim(),
  )
