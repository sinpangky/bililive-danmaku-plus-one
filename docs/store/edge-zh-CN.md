# bililive-danmaku-plus-one 浏览器商店资料（0.0.2）

## 单一用途描述

bililive-danmaku-plus-one 仅在 B 站直播中提供弹幕 `+1`、复制、回复、全屏发送、本地收藏和用户主动配置的独轮车循环发送。可选的侧边聊天栏胶囊提供同一组快捷操作且不遮挡消息正文；右侧聊天栏不会触发直播画面滚动弹幕选择。
