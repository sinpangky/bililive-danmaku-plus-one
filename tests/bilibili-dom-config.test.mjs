...
test('discovers plaintext-only fullscreen reply editors', () => {
  assert.equal(
    config.BILIBILI_QUICK_INPUTS.some((selector) =>
...
test('discovers current Bilibili image Emoji panels and pack containers', () => {
  assert.equal(
    config.BILIBILI_EMOJI_SURFACE_SELECTORS.some((selector) => selector.includes('emoji-box')),
    true,
  )
  assert.equal(
    config.BILIBILI_EMOJI_SURFACE_SELECTORS.some((selector) => selector.includes('emoticon-wrap')),
    true,
  )
  assert.equal(
    config.BILIBILI_EMOJI_SURFACE_SELECTORS.some((selector) => selector.includes('face-list')),
    true,
  )
})

test('recognizes prefixed text danmaku without treating its badge as Emoji', () => {
  assert.equal(config.BILIBILI_PREFIX_TEXT_SELECTOR, '.bili-danmaku-x-prefixImage-text-container')
})
